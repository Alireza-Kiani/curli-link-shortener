...
import type { Set, Get } from '../@types/response';

const { API_VERSION } = process.env;

class ApiService {
	async set(link: string): Promise<Set> {
		const raw_res_save = await fetch(
			`http://curli.ir:8083/api/v${API_VERSION}/set`,
			{
				method: 'POST',
				headers: {
		...
		return await raw_res_save.json();
	}

	async getRedis(url: string): Promise<Get> {
		const redis_raw_response = await fetch(
			`http://curli.ir:8083/api/v${API_VERSION}/${url}`
		);
